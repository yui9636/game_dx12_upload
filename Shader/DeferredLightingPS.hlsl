...

{
    ...
    float sceneDepth = DepthMap.Sample(PointSamp, pin.uv).r;
    ...
    float3 albedoLin = max(g0.rgb, float3(0.02f, 0.02f, 0.02f));
    ...
    if (any(isnan(probeColor)) || any(isinf(probeColor)))

    {

        probeColor = float3(0.0f, 0.0f, 0.0f);
    ...
    if (any(isnan(ssrColor)) || any(isinf(ssrColor)))

    {

        ssrColor = float3(0.0f, 0.0f, 0.0f);
    ...
    if (any(isnan(fogColor)) || any(isinf(fogColor)))
