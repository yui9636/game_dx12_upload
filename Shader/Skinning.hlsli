cbuffer CbSkeleton : register(b6)
{
    ...
    row_major float4x4	prevBoneTransforms[256];
};
...
float4 PrevSkinningPosition(float4 position, float4 boneWeights, uint4 boneIndices)
{
    float4 p = float4(0, 0, 0, 0);
    [unroll]
    for (int i = 0; i < 4; i++)
    {
        p += (boneWeights[i] * mul(position, prevBoneTransforms[boneIndices[i]]));
    }
    return p;
}