    ...
    , m_useDeviceFrameAllocator(useDeviceFrameAllocator)
{
    ...
    // 生成直後は recording 状態なので、一度 Close して Begin() で統一的に Reset する。
    m_commandList->Close();

    // フレーム中だけ使う SRV テーブルは専用 allocator で使い捨てる。
    ...
    // コマンドリスト単位で PSO キャッシュを持ち、再構築を減らす。
...
void DX12CommandList::Begin() {
    // フレーム開始時に allocator、descriptor heap、dynamic CB ring を初期状態へ戻す。
    ...
    // frame allocator を reset する。
    m_frameSrvAllocator->Reset();

    // descriptor heap を設定する。
...
void DX12CommandList::RestoreFrameDescriptorHeap() {
    // 外部 heap を使った後に、通常描画用のフレーム heap へ戻す。
    ID3D12DescriptorHeap* heaps[] = { m_frameSrvAllocator->GetHeap() };
    m_commandList->SetDescriptorHeaps(1, heaps);
}

void DX12CommandList::RestoreDescriptorHeap() {
    // ImGui 描画後は通常描画用の heap と root signature へ戻す。
    // root signature を張り直すと既存の root parameter が無効になるため、
    // フレーム途中の復帰には使わず、ImGui 後の最終復帰だけに限定する。
...
void DX12CommandList::PSSetSampler(uint32_t slot, ISampler* sampler) {
    // DX12 では sampler は root signature または別 heap に焼き込む。
    // 現状は no-op。完全実装では static sampler が担当する。
}
...
void DX12CommandList::SetInputLayout(IInputLayout* layout) {
    // DX12 では input layout が PSO の一部なので、PSO compile 用に保持する。
...
void DX12CommandList::SetRenderTargets(uint32_t numRTs, ITexture* const* rts, ITexture* depthStencil) {
    ...
    // PSO compile 用に RT / DSV format を追跡する。
...
ID3D11DeviceContext* DX12CommandList::GetNativeContext() {
    // DX12 には ID3D11DeviceContext 相当の object がない。
    return nullptr;
}
