...
void GamePad::Update()
{
	...
	if (XInputGetState(slot, &xinputState) == ERROR_SUCCESS)
	{
		...
		if (pad.wButtons & XINPUT_GAMEPAD_DPAD_UP)					ry = 1.0f;
		if (pad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT)				rx = 1.0f;
		if (pad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN)				ry = -1.0f;
		if (pad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT)				rx = -1.0f;
		...
		if (pad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)			newButtonState |= BTN_ESC;
		if (pad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)			newButtonState |= BTN_SPACE;
		...
		if (joyGetPosEx(slot, &joyInfo) == JOYERR_NOERROR)
		{
			...
			if (joyGetDevCaps(slot, &joy_caps, sizeof(JOYCAPS)) == JOYERR_NOERROR)
			{
				...
				if (joyInfo.dwButtons)
				{
					...
					if (joyInfo.dwButtons & JOY_BUTTON4)  newButtonState |= BTN_Y;
...

	{
		...
		if (GetAsyncKeyState('C') & 0x8000) newButtonState |= BTN_LEFT_SHOULDER;
